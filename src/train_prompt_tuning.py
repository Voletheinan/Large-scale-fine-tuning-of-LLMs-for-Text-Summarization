...
This script performs Prompt-tuning of the TinyLLaMA model for text summarization.
It loads the 4-bit quantized base model and applies Prompt-tuning adapters.
...
from peft import PromptTuningConfig, TaskType, get_peft_model, prepare_model_for_kbit_training
...
def main():
    print("Starting Prompt-tuning process...")
    ...
    print(f"Loading base model {MODEL_NAME} with 4-bit quantization for Prompt-tuning...")
    if torch.cuda.is_available():
    ...
    # 5. Prepare model for k-bit training and apply Prompt-tuning config
    ...
    # PromptTuningConfig needs TaskType to be passed directly for initialization
    peft_config = PromptTuningConfig(
        task_type=TaskType.CAUSAL_LM, 
        num_virtual_tokens=PROMPT_TUNING_CONFIG["num_virtual_tokens"],
        tokenizer_name_or_path=PROMPT_TUNING_CONFIG["tokenizer_name_or_path"],
    )
    model = get_peft_model(model, peft_config)
    print("Prompt-tuning model prepared successfully.")
    ...
    training_args = TrainingArguments(
        output_dir=P_TUNING_FINETUNED_MODEL_DIR,
        **TRAINING_ARGS,
        logging_dir=os.path.join(P_TUNING_FINETUNED_MODEL_DIR, "logs"),
    )
    ...
    print(f"Prompt-tuning complete. Training time: {training_time:.2f}s ({training_time/60:.2f} minutes)")
    ...
    # 8. Save the Prompt-tuning adapter
    print(f"Saving Prompt-tuning adapter to {P_TUNING_FINETUNED_MODEL_DIR}...")
    trainer.model.save_pretrained(P_TUNING_FINETUNED_MODEL_DIR)
    print("Prompt-tuning adapter saved successfully.")
    ...
    metrics_path = os.path.join(P_TUNING_FINETUNED_MODEL_DIR, "training_metrics.json")
    with open(metrics_path, 'w') as f:
        json.dump(metrics, f, indent=2)
    print(f"Training metrics saved to {metrics_path}")

    print(f"\n=== Prompt-tuning Training Summary ===")
