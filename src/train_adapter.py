...
This script performs Adapter (IA3) fine-tuning of the TinyLLaMA model
for text summarization. It loads the 4-bit quantized base model and applies IA3 adapters.
...
def main():
    print("Starting Adapter (IA3) Fine-tuning process...")
    ...
    print(f"Loading base model {MODEL_NAME} with 4-bit quantization for Adapter (IA3)...")
    if torch.cuda.is_available():
    ...
    # 5. Prepare model for k-bit training and apply IA3 config
    ...
    ia3_config = IA3Config(**IA3_CONFIG)
    model = get_peft_model(model, ia3_config)
    print("Adapter (IA3) model prepared successfully.")
    ...
    training_args = TrainingArguments(
        output_dir=ADAPTER_FINETUNED_MODEL_DIR,
        **TRAINING_ARGS,
        logging_dir=os.path.join(ADAPTER_FINETUNED_MODEL_DIR, "logs"),
    )
    ...
    print("Starting Adapter (IA3) Fine-tuning...")
    ...
    print(f"Adapter (IA3) Fine-tuning complete. Training time: {training_time:.2f}s ({training_time/60:.2f} minutes)")
    ...
    # 8. Save the IA3 adapter
    print(f"Saving IA3 adapter to {ADAPTER_FINETUNED_MODEL_DIR}...")
    trainer.model.save_pretrained(ADAPTER_FINETUNED_MODEL_DIR)
    ...
    metrics_path = os.path.join(ADAPTER_FINETUNED_MODEL_DIR, "training_metrics.json")
    with open(metrics_path, 'w') as f:
        json.dump(metrics, f, indent=2)
    print(f"Training metrics saved to {metrics_path}")

    print(f"\n=== Adapter (IA3) Training Summary ===")
